...
//! KBKDF (SP 800-108 counter-mode HMAC) derivation smoke tests.
//!
...
#[test]
fn test_kbkdf_aes_derive_smoke() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            ...
            let label = Some(MborByteArray::from_slice("label".as_bytes()).unwrap());
            let context = Some(MborByteArray::from_slice("context".as_bytes()).unwrap());
            let key_props =
            ...
            assert_eq!(
                ...
                plaintext,
                "KBKDF-derived AES key must round-trip encrypt/decrypt"
            );
        },
    );
}

/// Run an ECDH → KBKDF derivation into a sign/verify HMAC-family key
/// (`HmacSha*` or `VarHmac*`) and return the result.
fn kbkdf_hmac_derive(
    ...
    helper_kbkdf_derive(
        ...
        secret_key_id,
        DdiHashAlgorithm::Sha256,
        Some(MborByteArray::from_slice("label".as_bytes()).unwrap()),
        Some(MborByteArray::from_slice("context".as_bytes()).unwrap()),
        key_type,
        None,
        key_props,
        key_length,
    ...
    ddi_dev_test(
        ...
        |dev, _ddi, _path, session_id| {
            ...
            for key_type in [
                ...
                let resp = kbkdf_hmac_derive(dev, session_id, key_type, None);
                assert!(resp.is_ok(), "KBKDF {key_type:?} should derive: {resp:?}");
...
#[test]
fn test_kbkdf_var_hmac_derive_smoke() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            ...
            for (key_type, key_len) in cases {
                let resp = kbkdf_hmac_derive(dev, session_id, key_type, Some(key_len));
                assert!(
                    resp.is_ok(),
                    "KBKDF {key_type:?} len {key_len} should derive: {resp:?}"
                );
                let resp = resp.unwrap();
                assert_eq!(resp.hdr.op, DdiOp::KbkdfCounterHmacDerive);
                assert_eq!(resp.hdr.status, DdiStatus::Success);
...
#[cfg(not(feature = "mock"))]
#[test]
fn test_kbkdf_var_hmac_missing_length_smoke() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            // A var-len HMAC output requires an explicit key_length.
            let err = kbkdf_hmac_derive(dev, session_id, DdiKeyType::VarHmac256, None)
...
#[cfg(not(feature = "mock"))]
#[test]
fn test_kbkdf_var_hmac_out_of_range_length_smoke() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            // 16 is below VarHmac256's 32-byte minimum.
            let err = kbkdf_hmac_derive(dev, session_id, DdiKeyType::VarHmac256, Some(16))
