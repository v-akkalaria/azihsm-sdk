...
//! intent at the call site (`assert_fw_rejects(err, HsmError::…)`)
...
/// the given [`HsmError`] discriminant.
///
/// The TBOR FW dispatcher encodes such rejections via
/// `encode_tbor_err`, which writes the `HsmError.0` u32 into the
...
#[track_caller]
pub fn assert_fw_rejects(err: &DdiError, expected: HsmError) {
    let expected_code = expected.0;
    match err {
        DdiError::DdiError(code) => assert_eq!(
            *code, expected_code,
            "FW rejected with wrong HsmError: expected {expected:?} (0x{expected_code:08X}), \
             got 0x{code:08X}",
        ),
        other => panic!(
